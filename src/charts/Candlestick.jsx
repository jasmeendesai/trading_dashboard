...
function CandleStick({sym, interval}) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://finnhub.io/api/v1/stock/candle?symbol=${sym}&resolution=${interval}&from=1693493346&to=1693752546&token=${apikey}`);
      ...
      const newChartData = {
        ...
        options: {
          ...
          xaxis: {
            ...
            categories: t.map((timestamp) => new Date(timestamp * 1000).toLocaleTimeString()), 
          },
        },
      };
